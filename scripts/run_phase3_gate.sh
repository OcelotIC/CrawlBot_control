...
} > results/phase3_wp/PHASE3_METADATA.txt
SSMOM_RUN_DIR=phase3_wp MUJOCO_GL=disabled PYTHONPATH=. python3 scripts/postprocess_results_figs.py \
  > "$SP/phase3_wp_postproc.log" 2>&1
echo "  WP postproc exit=$?  F3F4=$(test -f results/phase3_wp/postproc_F3F4.csv && echo YES || echo NO)"
...
echo "  $TQ"
echo "test_reworked_qp: $TQ" >> results/phase3_off/OFF_RESULT.txt
