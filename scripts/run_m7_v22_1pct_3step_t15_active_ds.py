...
OUT = os.path.join(_root, 'results', 'M7_1pct_3step_v22_t15_active_ds')
...
def main():
    ...
    print(f'[T15-ACTIVE-DS] MJCF md5 (pre-run, pre-mutation):  {pre_hash}')
    try:
        _mutate_mjcf(damping=0.0, armature=0.05)
        mid_hash = _mjcf_md5(MJCF)
        print(f'[T15-ACTIVE-DS] MJCF md5 (during run, mutated): {mid_hash}')
        ...
        # Active-DS torso advance (active_ds_torso_advance.md). With
        # ds_active_enabled, inter-step DS phases run a torso-advance
        # loop instead of the legacy passive settle, so SS-entry has
        # the body pre-positioned ~70%% toward the next anchor.
        cfg.ds_active_enabled = True
        cfg.ds_active_beta = 0.7

        print(f'[T15-ACTIVE-DS] reference_source                = '
              f'{cfg.reference_source!r}')
        print(f'[T15-ACTIVE-DS] geodesic_n_tau / n_iter / tol   = '
              f'{cfg.geodesic_n_tau} / {cfg.geodesic_n_iter} / '
              f'{cfg.geodesic_tol:.1e}')
        print(f'[T15-ACTIVE-DS] use_trajectory_aware_ik         = '
              f'{cfg.use_trajectory_aware_ik}')

        sim, log, metrics = r_single.run_case(
            '1pct 3-step v22 T15-ACTIVE-DS '
        ...
        print(f'[T15-ACTIVE-DS] MJCF md5 (post-run, restored):   {post_hash}')
