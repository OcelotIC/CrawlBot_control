...
- **12 pytest problems (9 failed + 3 errors), and 11 of the 12 are this chantier's own breakage** — not pre-existing, as CLEANUP-25/26/27 first recorded. Measured at `4e2e8da^` in a clean worktree: `test_reworked_qp.py` **8 passed**. All 8 now die on one line (`tests/test_reworked_qp.py:87`, `TypeError: unexpected keyword argument 'alpha_com'`) because CLEANUP-6/9 deleted 9 config fields the shared `_build_qp` helper still passes; no test body runs. Plus `test_mid_waypoint_reshape` ×3 and `test_fk_reference_consistency` (CLEANUP-15/17 removals). **Only `test_coarse_preplanner::test_far_infeasible_under_tight_rate` predates the chantier.** Diagnosis and the fix (retire T7–T10, port `TestPhase20TMOM` ×4 to the two-task API) in `PHASE_CLEANUP_27_TEST_SUITE_AUDIT.md` §4. The suite is **not** gated — `gate/run_gate.py` is, which is why this drifted.
