...
| NMPC horizon N | **20** (8 → 15 → 20; 2.0 s lookahead = 33 % of T_step). **Now a clean knob** — F1 decoupled the reference sampling from N | — | `config.py:265`, spec §5.1 |
...
| **α torso-pose** | **2000** | — | `config.py:342` (Add-5) |
| **α swing-EE** | **1000** (dock lever; needs ≥ ~1000) | — | `config.py:321` (Add-5) |
| **α momentum (T-MOM)** | **400** (near-inert on Ḣ_s — NMPC owns the envelope) | — | `config.py:329` (Add-5) |
| **w hw-slack** | **800** (slacks active only if the hw box is violated) | — | `wholebody_qp.py:218` (Add-5) |
| **α posture** | **20** | — | `config.py:322` |
| **α torque-min** | **5** (must stay ≳ 5× accel-reg floor — Rule 14) | — | `sim_loop.py:972` (QP-construction literal) |
| **α wrench-track** | **1.0** | — | `config.py:323` (Add-5; was 0.01 pre-freeze) |
