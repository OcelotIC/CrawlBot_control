...
- **⚠ The NMPC h_w box bounds an ESTIMATE whose error can exceed its own margin.** Spec §3.5's exact law carries `(I_s + I_robot)·ω_s`; Options A/B freeze the platform, absorbing `I_s·ω_s0` into `c`, so the residual is `I_s·(ω_s(k) − ω_s(0))`. Measured over one 2.0 s horizon: **x 1.44, y 1.90, z 1.27 Nms** against box margins 4.42 / 2.66 / **1.19** — i.e. **z is 107 % of its margin**. The spec's prescribed allowance `h_max' = h_max − ε_drag` covers only the *Option-B* term (`ε_drag ≈ 4.7e-3 Nms`, negligible) and **was never applied anyway** (`h_max_tight = hw_max = 5.0`, zero tightening). Root cause: the spec justified the neglect at **mass ratio ~14 %**; the canonical runs at **1 %**, so `ω_s` is 37× smaller than assumed but `I_s` is far larger — it bounded `ω_s`, never the product `I_s·ω_s`. The traversal is sound (h_w peaks 3.815 < 5), but calling the box an envelope *guarantee* over-claims. Options and a recommendation: `results/j2_adjconv/NMPC_CONSERVATION_DRAG.md` §6. **Idriss's call — it changes the binding set, so not a silent retune.**
- **`h_w` is a within-step EXCURSION, not a traversal-scale wind-up — the box is a transient bound.** Net drift over the full 6-step run is x **+0.001**, y **+0.098**, z **−0.239** Nms against a z peak of **−3.815** (16× the drift): the wheels take on momentum to reject a contact wrench and hand it back when the step reverses. Consistent with `T·τ_w,max = h_max` exactly — box and rate cap describe the same 2.0 s window, so there is no longer timescale on which the level bound could speak. **Consequence for any "make the box bite" experiment:** a longer lever (robot further from the structure CoM) raises `Ḣ_s` *demand*, but the delivered rate is already clipped — z's **95th percentile IS the 2.5 cap**, 302/1967 ticks at it. What grows is the *duration* of saturation, which `h_w` integrates (z needs 2.00 s continuous vs 1.53 s today, +31 %). But every saturated second is a second the wheels do not reject the moment, so `ω_s` — and with it the `I_s·Δω_s` reconstruction error above — grows by the same mechanism on the same axis. **Binding the box and degrading its estimate share a driver**, so the export gaps below are a precondition, not a nicety. `results/j2_adjconv/NMPC_F2_RWA_BOX.md` §2.4; curves `nmpc_sweep/nmpc_momentum_budget.png`.
- **⚠ REFINES the entry above: the CRAWL does carry on z at −0.314 Nms/step — the 20 s terminal settle is what hides it.** The "net drift ≈ 0" figure includes `DS_terminal` (20.0 s), which returns **+0.999 Nms** on z, 64 % of the crawl carry. `DS_interstep` between crawl steps is only 2.0–3.2 s — too short to dump what the SS took on — so each step hands the next a more negative start. SS entries walk down monotonically: −0.029, −0.122, −0.809, −0.780, −1.071, −1.598. Carry per step x −0.020 (|mean|/std 0.36, zero-mean), y +0.150 (0.87, zero-mean), **z −0.314 (1.18, SYSTEMATIC)** — but n = 5, ≈2.6σ, **suggestive not conclusive**. Extrapolating `peak(N) ≈ |carry|·N + excursion`: z reaches ±5 at **~7 steps ≈ 2.6 m** of CoM travel (mean step 0.367 m), so a 20 m path is ~54 steps, **8× more than needed**. **`compute_c_simple` is re-anchored EVERY solve** (`centroidal_nmpc.py:699`, inside `_assemble_params`), so `I_s·Δω_s` accrues over one horizon and does NOT grow with path length — making "more steps" the one route to a binding box that does not degrade the estimate (the longer-lever route does, §2.4). `results/j2_adjconv/NMPC_F2_RWA_BOX.md` §2.5; curves `nmpc_sweep/nmpc_hw_per_step.png`.
- **`v_s` (platform linear velocity) is NOT in the fulldiag export**, so `ε_drag` can only be bounded from structure-drift telemetry. Adding `v_struct_*` and the reconstruction residual `ĥ_w − h_w` would make the above a per-run diagnostic — and without the residual, a run where the box appears to bind cannot be distinguished from one where the estimator drifted into it.
- **⚠ RETRACTED: NMPC_AUDIT F2 ("the RWA conservation box is OFF") was WRONG — the box AND its terminal set have always been ON** (`ng_path=17`, `ng_term=6`, `h_max_tight=±5` per axis). The audit built its structural probe from a bare `SimConfig()`, but `dca.main` uses `run_m7_single_step._make_m7_config()` (`diag_cooperative_arms.py:268`), which passes `enforce_hw_conservation=True`, `h_max_tight=5.0`, `kappa_terminal=1.0` as **explicit kwargs**. **Consequence for any future experiment: patching a `SimConfig` DEFAULT does nothing for a field `_make_m7_config()` sets explicitly** — that list is `use_m2_stack`, `alpha_passivity`, `enforce_hw_conservation`, `h_max_tight`, `w_L_nmpc`, `kappa_terminal`, `aocs_mode`, `aocs_use_legacy_corrected`, `aocs_use_H_estimator`, `preplanner_*`. Assert the value from INSIDE the run, not from the file. `scripts/audit_nmpc_structure.py` now builds from `_make_m7_config()`. Retraction: `results/j2_adjconv/NMPC_F2_RWA_BOX.md` §0.
- **The `centroidal_nmpc.md` §3 "stale doc" claim is itself retracted — that doc was RIGHT.** — it claimed the canonical sets `enforce_hw_conservation=True` with `ng_path=17`/`ng_term=6`, presented as "measured on the real run". Measured now: **False**, `ng_path=11`, `ng_term=0`. Corrected in place. A stale doc that states a measurement is worse than one that states nothing.
