...
- **`v_s` (platform linear velocity) is NOT in the fulldiag export**, so `ε_drag` can only be bounded from structure-drift telemetry. Adding `v_struct_*` and the reconstruction residual `ĥ_w − h_w` would make the above a per-run diagnostic.
