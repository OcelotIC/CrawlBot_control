...
empirical tell reuses the committed `results/fixA_gate/sim_log.json`). Reproducer `scripts/audit_aocs_ds.py`
...
## For how long — empirical (results/fixA_gate, full 5-step traversal)
...
`results/fixA_gate/sim_log.json` (committed). No `crawlbot/` change ⇒ no regression run needed.
