...
|---|---|---|---|---|
| H1 | **Internal/squeeze force** — two contacts simultaneously large & opposing, living in the null space of the 12→6 centroidal map (soft-CoM cannot touch it). | λ decomposition at the peak (`Misc/scripts/diag_lambda_decomp_step2.py`, pure analysis). | **Contact-2 force ≡ 0 N across all 89 step-2 SS ticks** (max 0.000 N); only the single stance weld is active. A single 6D contact has **no internal null space** → internal force = 0 by construction. `|f1|=62.2 N`, independently cross-checked by `m·\|a_com\|=45.8 N` (CoM is genuinely accelerating). | **REFUTED.** The 62 N is **net** wrench, not internal. |
...
- **Probe 1 (reference smoothness, `Misc/scripts/probe1_reference_smoothness.py`).**
...
- **Probe 2 (constrained conditioning, `Misc/scripts/probe2_cmm_conditioning.py`).**
...
- `Misc/scripts/diag_lambda_decomp_step2.py` → `Misc/runs/diag_attribution/lambda_decomp/{metrics.json, lambda_decomp.png}`
- `Misc/scripts/diag_loopfree_mapping_step2.py` → `Misc/runs/diag_attribution/loopfree_mapping/{sim_log.json, compare.json}`
- `Misc/scripts/probe1_reference_smoothness.py` → `Misc/runs/diag_attribution/probe1_reference_smoothness/{metrics.json, probe1.png}`
- `Misc/scripts/probe2_cmm_conditioning.py` → `Misc/runs/diag_attribution/probe2_cmm_conditioning/{metrics.json, probe2.png}`
