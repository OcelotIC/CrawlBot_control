...
Diagnostics: `Misc/scripts/diag_dock_quality.py`.
...
## 5. Group B — momentum / AOCS health (`Misc/scripts/diag_momentum_aocs.py`)
...
## 6. Group D — actuator / solver health (`Misc/scripts/diag_actuator_solver.py`)
...
## 6b. Group C — cascade band-aid footprint (`Misc/scripts/diag_cascade_health.py`)
...
1. **The only prior sweep used the wrong stack.**
   ...
   `Misc/scripts/sweep_alpha_com_soft.py`, which builds a plain `SimConfig`
...
## 9. Group Ḣ_s — proxy-vs-exact wheel-feasibility (`Misc/scripts/diag_hdot_struct.py`)
...
or `--tau_struct_max 5.0` (constraint on, opt-in); then `python3 Misc/scripts/diag_hdot_struct.py [subdir]`.
...
then `python3 Misc/scripts/diag_settle.py diag_cooperative_arms_legacy_pd_numerical`.
...
2. **Ḣ feedforward is finite-difference, not analytical**
   (`Misc/scripts/diag_hdot_feedforward.py`, post-PR-#20 diagnostic). The
