...
**Input artefacts:**
- `Misc/runs/archive_rediagnostic.md` — archived v21 value (`DS peak ori = 45.4672°`, SS peak `0.5348°`).
- `Misc/runs/M7_abort_diag/R1_baseline/sim_log.json` — committed in `6128db9`.
- `Misc/reports/architecture/M7_DS_DIAGNOSTIC_EXPERIMENTS.md` — task spec §4/§5.

## 1. Five-value reproduction table

All values are `torso_ori_peak_deg_DS` from the per-phase refactor of `compute_metrics`. Simulation setup: 1% mass ratio, `n_steps=1`, `start_a=2`, `start_b=2`, v21 config (`preplanner_a_cruise_max=0.01`, `preplanner_cruise_ramp_frac=0.2`, all other M7 settings as in `Misc/scripts/run_m7_v21_preplanner_cruise.py`).

| # | scenario | DS peak ori [°] |
|---|---|---|
| 1 | archived v21 (`Misc/runs/M7_1pct_1step_v21/`, 2026-04-17) | 45.4672 |
...
Each scenario was executed twice on this machine today — once to `Misc/runs/M7_abort_diag/` (committed in `6128db9`, "run 1") and once to a scratch directory (discarded, "run 2"). Identical config, identical machine, back-to-back runs.
...
2. **On this machine today, every R* scenario is deterministic across repeats.** All four DS peak values in §2 are bit-identical between run 1 and run 2. SS-side metrics are also bit-identical. The R1–R4 deltas reported in `Misc/runs/M7_abort_diag/summary.md` therefore reflect the diagnostic flag effects, not run-to-run noise.

3. **SS-side behaviour matches the 2026-04-17 archive bit-for-bit.** SS peak ori (`0.5334°`), `ss_end_torso_ori_deg` (`0.1990°`), `q_torso_ref_ss_to_ds_jump_deg` (`3.4162°`), and the abort event (`d_mm = 40.8371`, `ori_deg = 6.9671`) all reproduce today with or without the flags, and match the archived v21 values from `Misc/runs/archive_rediagnostic.md`.
...
The four-run R1–R4 data live under `Misc/runs/M7_abort_diag/` at commit `6128db9`. Together with `Misc/scripts/run_m7_abort_diag.py` (same commit) and the v21 config produced by `scripts/run_m7_single_step._make_m7_config()` + `preplanner_a_cruise_max=0.01` + `preplanner_cruise_ramp_frac=0.2`, they fully regenerate the §1 rows 2–5 and the §2 table on the same machine/toolchain.

The raw sim_logs and diagnostic plots from the scratch reruns used to populate §1 rows 3, 5 and the "run 2" columns of §2 were **not preserved on the branch**. The canonical R1–R4 artefacts under `Misc/runs/M7_abort_diag/` (commit `6128db9`) are preserved. Given determinism on this machine (§3.2), the scratch logs can be regenerated exactly from the referenced scripts, so keeping the duplicates adds nothing over the numerics in this report.
