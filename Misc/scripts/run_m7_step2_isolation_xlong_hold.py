...
OUT_ROOT = os.path.join(_root, 'Misc', 'runs', 'M7_step2_isolation_xlong_hold')
...
def main():
    ...
    variants = [
        ('xlong_hold', True, 't_ss_margin_20.0'),
    ]
    results = {}
    ...
    rep_path = os.path.join(OUT_ROOT, 'STEP2_XLONG_HOLD_REPORT.md')
    with open(rep_path, 'w') as fp:
        ...
        for k in ['docked', 'd_grip_swing_min_mm', 'd_grip_swing_final_mm',
            ...
            fp.write(f"| {k} | {v} |\n")
        fp.write(f"\n## Figures\n\n")
        fp.write(f"- `xlong_hold/fig_qp_isolation_tracking.png` — torso/EE refs vs actual\n")
        fp.write(f"- `xlong_hold/fig_qp_isolation_actuators.png` — joint, AOCS torque, AOCS momentum\n")
        fp.write(f"- `xlong_hold/fig{{1..10}}_*.png` — full diagnostic suite\n")
    print(f"\nReport: {rep_path}")
    return 0
