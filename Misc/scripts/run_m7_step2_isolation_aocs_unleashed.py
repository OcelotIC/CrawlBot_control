...
Run:
  PYTHONPATH=. MUJOCO_GL=disabled python3 \\
      Misc/scripts/run_m7_step2_isolation_qp_test.py
"""
...
# is the actual binding constraint at clean-start step 2.
AOCS_TAU_W_MAX_UNLEASHED = 100.0
AOCS_HW_MAX_UNLEASHED = 100.0
...
def _make_step2_isolation_config(bypass_on: bool) -> SimConfig:
    ...
    # Lift the AOCS torque + momentum budgets to confirm whether AOCS
    # saturation (5.5 Nm peak observed at clean-start step 2) is the
    # binding constraint. 20× the physical envelope.
    cfg.tau_w_max = AOCS_TAU_W_MAX_UNLEASHED
    cfg.aocs_tau_w_max = AOCS_TAU_W_MAX_UNLEASHED
    cfg.hw_max = np.full(3, AOCS_HW_MAX_UNLEASHED)
    cfg.hw_min = np.full(3, -AOCS_HW_MAX_UNLEASHED)
    return cfg
...
def main():
    ...
    # Single variant — under reference_source='joint_space_fk' the FK
    # branch fires regardless of mapping_bypass_in_ss, so the previous
    # A/B distinction had no effect. Use bypass=on (current default)
    # and lift AOCS budget to 20× nominal.
    variants = [
        ('aocs_unleashed', True, 'aocs_100Nm_100Nms'),
    ]
    results = {}
    for sub, bypass_on, label in variants:
        ...
        print(f"\n{'='*72}")
        print(f"  variant {sub}: bypass={'ON' if bypass_on else 'OFF'}, "
              f"start=(3, 3), n_steps=1, swing arm goes 3 → 4")
        print(f"{'='*72}")
        sim, log, metrics = r_single.run_case(
            f'STEP2-ISOLATION variant={sub} '
            f'(reference_source=joint_space_fk, mapping_bypass_in_ss={bypass_on})',
    ...
    rep_path = os.path.join(OUT_ROOT, 'STEP2_QP_ISOLATION_REPORT.md')
    with open(rep_path, 'w') as fp:
        fp.write("# Step-2 QP isolation test — report\n\n")
        fp.write(f"**Branch:** `claude/fk-bypass-aware-tuning`\n\n")
        fp.write(f"**Test scenario:** `start_a=3, start_b=3, n_steps=1` — "
                 f"system initialised at (3, 3) docked configuration; swing "
                 f"arm 'b' commanded to anchor 4. **No prior drift from "
                 f"steps 0/1.**\n\n")
        fp.write("## Side-by-side metrics\n\n")
        fp.write("| metric | A: bypass=ON (current default) | B: bypass=OFF (FK ref to QP) |\n")
        fp.write("|---|---|---|\n")
        keys = ['docked', 'd_grip_swing_min_mm', 'd_grip_swing_final_mm',
        ...
        for k in keys:
            a = results['A_bypass_on'].get(k, '—')
            b = results['B_bypass_off'].get(k, '—')
            ...
            fp.write(f"| {k} | {a} | {b} |\n")
        fp.write("\n## Verdicts\n\n")
        for sub in ['A_bypass_on', 'B_bypass_off']:
            fp.write(f"- **{sub}**: {_verdict_for(results[sub])}\n")
        fp.write(f"\n## Figures\n\n")
        for sub in ['A_bypass_on', 'B_bypass_off']:
            fp.write(f"- `{sub}/fig_qp_isolation_tracking.png` — torso/EE refs vs actual + tracking errors\n")
            fp.write(f"- `{sub}/fig_qp_isolation_actuators.png` — joint torque, AOCS torque, AOCS momentum\n")
        fp.write(f"\n## Standard diagnostic figures (per variant)\n\n")
        fp.write(f"- `{{A_bypass_on,B_bypass_off}}/fig{{1..10}}_*.png` — full diagnostic suite\n")
    print(f"\nReport: {rep_path}")
    return 0
