...
OUT = os.path.join(_root, 'results', 'M7_1pct_3step_v22_t15_fk')
...
def main():
    ...
    print(f'[T15-FK] MJCF md5 (pre-run, pre-mutation):  {pre_hash}')
    try:
        _mutate_mjcf(damping=0.0, armature=0.05)
        mid_hash = _mjcf_md5(MJCF)
        print(f'[T15-FK] MJCF md5 (during run, mutated): {mid_hash}')
        ...
        print(f'[T15-FK] MJCF md5 (post-run, restored):   {post_hash}')
