...
OUT = os.path.join(_root, 'Misc', 'runs', 'M7_1pct_3step_v22_t15_fk_aocs_off')
...
def main():
    ...
    print(f'[T15-FK-AOCS-OFF] MJCF md5 (pre-run, pre-mutation):  {pre_hash}')
    try:
        _mutate_mjcf(damping=0.0, armature=0.05)
        mid_hash = _mjcf_md5(MJCF)
        print(f'[T15-FK-AOCS-OFF] MJCF md5 (during run, mutated): {mid_hash}')
        ...
        print(f'[T15-FK-AOCS-OFF] reference_source                = '
              f'{cfg.reference_source!r}')
        print(f'[T15-FK-AOCS-OFF] geodesic_n_tau / n_iter / tol   = '
              f'{cfg.geodesic_n_tau} / {cfg.geodesic_n_iter} / '
              f'{cfg.geodesic_tol:.1e}')
        print(f'[T15-FK-AOCS-OFF] use_trajectory_aware_ik         = '
              f'{cfg.use_trajectory_aware_ik}')

        sim, log, metrics = r_single.run_case(
            '1pct 3-step v22 T15-FK-AOCS-OFF '
            '(reference_source=joint_space_fk; AOCS DISABLED)',
            OUT, n_steps=3, config=cfg, diag_disable_aocs=True)
        ...
        print(f'[T15-FK-AOCS-OFF] MJCF md5 (post-run, restored):   {post_hash}')
