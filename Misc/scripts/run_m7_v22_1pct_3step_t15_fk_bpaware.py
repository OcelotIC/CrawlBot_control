...
(Misc/scripts/run_m7_v22_1pct_3step_t15_ik_fix.py) except for:
...
OUT = os.path.join(_root, 'Misc', 'runs', 'M7_1pct_3step_v22_t15_fk_bpaware')
...
def main():
    ...
    print(f'[T15-FK-BPAWARE] MJCF md5 (pre-run, pre-mutation):  {pre_hash}')
    try:
        _mutate_mjcf(damping=0.0, armature=0.05)
        mid_hash = _mjcf_md5(MJCF)
        print(f'[T15-FK-BPAWARE] MJCF md5 (during run, mutated): {mid_hash}')
        ...
        # v3 ablation: KEEP baseline gains (Kp=6, Kd=5). The fix this
        # time is in sim_loop.py — under FK mode we now zero out the
        # linear velocity/acceleration feedforwards (since the FK ref's
        # piecewise-constant v_full produces impulsive a_ff at sample
        # boundaries, which the gain stack can't filter). The QP does
        # pure-PD on the smoothly-interpolated FK position ref. v1 (Kp
        # uniform 3) and v2 (Kp_lin=4, Kp_ang=6) showed gain softening
        # alone can't overcome the discontinuous feedforwards.
        cfg.ss_Kp_torso = 6.0
        cfg.ss_Kd_torso = 5.0

        print(f'[T15-FK-BPAWARE] reference_source                = '
              f'{cfg.reference_source!r}')
        print(f'[T15-FK-BPAWARE] geodesic_n_tau / n_iter / tol   = '
              f'{cfg.geodesic_n_tau} / {cfg.geodesic_n_iter} / '
              f'{cfg.geodesic_tol:.1e}')
        print(f'[T15-FK-BPAWARE] use_trajectory_aware_ik         = '
              f'{cfg.use_trajectory_aware_ik}')

        sim, log, metrics = r_single.run_case(
            '1pct 3-step v22 T15-FK-BPAWARE '
        ...
        print(f'[T15-FK-BPAWARE] MJCF md5 (post-run, restored):   {post_hash}')
