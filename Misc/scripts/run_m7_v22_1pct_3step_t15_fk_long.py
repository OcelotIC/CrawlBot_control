...
OUT = os.path.join(_root, 'Misc', 'runs', 'M7_1pct_3step_v22_t15_fk_long')
...
def main():
    ...
    print(f'[T15-FK-LONG] MJCF md5 (pre-run, pre-mutation):  {pre_hash}')
    try:
        _mutate_mjcf(damping=0.0, armature=0.05)
        mid_hash = _mjcf_md5(MJCF)
        print(f'[T15-FK-LONG] MJCF md5 (during run, mutated): {mid_hash}')

        cfg = r_single._make_m7_config()
        # Baseline T15 settings (byte-identical to run_m7_v22_1pct_3step_t15.py
        # and IK-fix runner).
        cfg.preplanner_a_cruise_max = 0.005  # halve cruise accel → roughly 1.4× T_step
        cfg.preplanner_cruise_ramp_frac = 0.2
        cfg.t_ss_margin = 5.0  # extra grace time beyond T_step (default 1.0)
        ...
        print(f'[T15-FK-LONG] reference_source                = '
              f'{cfg.reference_source!r}')
        print(f'[T15-FK-LONG] geodesic_n_tau / n_iter / tol   = '
              f'{cfg.geodesic_n_tau} / {cfg.geodesic_n_iter} / '
              f'{cfg.geodesic_tol:.1e}')
        print(f'[T15-FK-LONG] use_trajectory_aware_ik         = '
              f'{cfg.use_trajectory_aware_ik}')

        sim, log, metrics = r_single.run_case(
            '1pct 3-step v22 T15-FK-LONG '
        ...
        print(f'[T15-FK-LONG] MJCF md5 (post-run, restored):   {post_hash}')
