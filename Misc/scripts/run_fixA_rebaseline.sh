...
#      Misc/runs/fixA_gate/  -> six criteria
set -u
cd "$(dirname "$0")/.."
...
SSMOM_RUN_DIR=ssmom_phase1_baseline_fixA MUJOCO_GL=disabled PYTHONPATH=. \
  python3 Misc/scripts/postprocess_results_figs.py > "$SP/baseline_fixA_pp.log" 2>&1
echo "  ref postproc exit=$? metrics=$(test -f Misc/runs/ssmom_phase1_baseline_fixA/postproc_metrics.json && echo Y || echo N)"
...
SSMOM_RUN_DIR=phase3_off MUJOCO_GL=disabled PYTHONPATH=. \
  python3 Misc/scripts/postprocess_results_figs.py > "$SP/phase3_off_pp.log" 2>&1
...
echo "  $TQ"; echo "test_reworked_qp: $TQ" >> Misc/runs/phase3_off/OFF_RESULT.txt
...
SSMOM_WP_DIR=fixA_gate SSMOM_OFF_DIR=phase3_off MUJOCO_GL=disabled PYTHONPATH=. \
  python3 Misc/scripts/gate_phase3.py 2>&1 | grep -aE 'C[1-6]|per-step|NON-binding|BINDING|B15'
echo "=== done (Phase 1) ==="
