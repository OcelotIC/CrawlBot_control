...
for root, dirs, files in os.walk('crawlbot'):
    ...
    for f in sorted(files):
        ...
        pct = f"{entry['summary']['percent_covered']:.0f} %" if entry else 'never imported'
        ...
        L = [f'# `crawlbot.{pkg}.{mod}`', '',
             '<!-- PROSE: one-line summary -->', '',
             f'**File**: `{p}` — **{nlines} lines** — '
             f'canonical coverage **{pct}**', '']
        if head:
            L += [f'> Module docstring: *"{head}"*', '']
        L += ['---', '', '## Public API', '',
        ...
        L += ['', '---', '', '<!-- PROSE: how it works, maths, traps -->',
              '', '## See also', '',
              f'- package overview: [`{pkg}.md`]({pkg}.md)', '']
