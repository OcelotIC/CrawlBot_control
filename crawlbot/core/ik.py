...
Provides:
    ...
    dock_configuration_fixed_rotation — IK 1, level-torso variant
    manipulability_config — IK 2, endpoint-only optimal-manipulability config
                            for a dock pair. THE path sim_loop uses
                            (sim_loop.py:307). IK_FORMULATION.md §6.
    solve_ik_waypoints    — chain of IK solutions along a swing arc.
                            No callers anywhere (CLEANUP-30 measurement).

CLEANUP-30 retired IK 3, the Option-B trajectory-aware path — 695 lines, 47 %
of this file: manipulability_config_trajectory, manipulability_config_mid_waypoint,
check_path_feasibility, precompute_torso_map, and the four helpers that stranded
with them. Zero callers in crawlbot/, and 0 lines executed by the canonical
replay. IK_FORMULATION.md §7-§9 still derives them and carries a banner saying
they are gone; revival starts at `git show d61e1a0:crawlbot/core/ik.py`, with
the regression tests kept in Misc/tests/.
"""
...
def dock_configuration_fixed_rotation(
    ...
    w_sigma_min : float
        ...
        worst-direction manipulability that IK 3 used as its cost
        before CLEANUP-30 retired it. Still reported here as a
        secondary diagnostic for cross-comparison; resolves the
        metric-mismatch artefact identified in
        IK_ANOMALY_REPORT.md §4.3.
    """
